...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    docker cp -a orderer0.example.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer0.example.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer0.example.com
